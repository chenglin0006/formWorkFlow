...
class LoginForm extends Component {
  ...
  static defaultProps = {
    ...
    modalType: 'login', // login表示是登录表单，bind表示是绑定并登录表单
  };
  ...
  componentDidMount() {
    const { showThirdLogin, modalType, stayInLogin, saveUnlock, saveType } = this.props;
    // 页面进来收纳账号密码登陆
    saveUnlock(false);
    // 一进来选择免密登陆
    let type = 'mobile';
  ...
  onSubmit = (e) => {
    ...
    this.formRef.current.validateFields(activeFileds).then(async (values) => {
      let flag = true;
      if (!this.state.loginCheckFlag && modalType !== 'bind') {
  ...
  postLogin = async (data = {}) => {
    const { login, modalType } = this.props;
    ...
    if (modalType === 'bind' && res.success && res.data && res.data.hasChangedPwd) {
      message.success('绑定账号成功');
    }

    if (modalType === 'login' && !res.success) {
      if (res.code === 402) {
        Modal.confirm({
  ...
  handleBlur = async (value) => {
    const { modalType, loginCheck, type } = this.props;
    if (modalType === 'login') {
  ...
  render() {
    const { type, unlock, submitLoading, modalType, showThirdLogin } = this.props;
        ...
        <div className="bnq-login-form">
          <Form ref={this.formRef} autoComplete="off">
            ...
            <Tabs className="bnq-login-tabs" activeKey={type} onChange={this.onChangeType}>
              ...
              <TabPane tab={`${modalType === 'bind' ? '快捷绑定' : '免密登录'}`} key="mobile">
                {this.alertCpn()}
              ...
              {(unlock || modalType === 'bind') && (
                <TabPane tab={`密码${modalType === 'bind' ? '绑定' : '登录'}`} key="account">
                  {this.alertCpn()}
